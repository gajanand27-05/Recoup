...
def section_7_limits() -> None:
    ...
    say("* The reply classifier calls a real model (gemini-2.5-flash) and its")
    say("  accuracy is NOT yet measured. The eval is written — 60 hand-labelled")
    say("  fixtures — but the free tier allows 20 requests per DAY, so it")
    say("  cannot complete. Attempted today; it got 20 in and stopped.")
    say("  I would rather report that than a partial accuracy over 20 items.")
    say("  There is deliberately no stub fallback: a stand-in producing output")
    say("  that reads like a real run is the failure mode every guard here")
    say("  exists to prevent.")
    say("* For the same reason the batch run will NOT call the model per reply.")
    say("  The lift figure is not a claim about the model's accuracy.")
